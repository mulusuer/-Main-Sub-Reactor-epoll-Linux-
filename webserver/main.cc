...
// 静态资源根目录
const std::string ROOT_PATH = "../static";
               ...
               HttpResponse* resp)
{
    ...
    // ================== 【新加：图片资源处理】 ==================
    else if (req.path().substr(0, 8) == "/images/")
    {
        std::string imgPath = ROOT_PATH + req.path();
        std::string image = readFile(imgPath);
